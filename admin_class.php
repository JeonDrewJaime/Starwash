...
Class Action {
	...
	function save_user(){
		...
		// Add the email to the data being saved
		...
		if($id == ''){
			...
			$qry = $this->db->query("SELECT * FROM users where username = '".$username."' OR email = '".$email."'");
			if($qry->num_rows > 0){
				return 3;  // Username or email already exists
			}
			// Insert new user
